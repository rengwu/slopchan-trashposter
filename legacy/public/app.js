...
let config, state, session, availability, presets, dirty = false, busy = false, tab = 'personalities', editing, detailId, toastTimer, connected = false;
...
const locked = () => !!(state?.active || state?.running || busy);
...
async function api(path, body = {}) { const r = await fetch(`/api/${path}`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-Trashposter-Session': session }, body: JSON.stringify(body) }); const data = await r.json(); if (!r.ok) throw new Error(data.error || `HTTP ${r.status}`); return data; }
function formToConfig() {
  config.url = $('#url').value.trim(); config.prompt = $('#prompt').value; config.dryRun = $('#dryRun').checked; config.posting = $('#posting').value;
  for (const key of ['interval', 'min', 'max', 'timeout']) config.schedule[key] = Number($(`#${key}`).value);
...
function renderConfig() {
  for (const key of ['url', 'prompt', 'posting']) $(`#${key}`).value = config[key];
  for (const key of ['interval', 'min', 'max', 'timeout']) $(`#${key}`).value = config.schedule[key];
...
function renderClock() {
  ...
  $('#interval-readout').textContent = random ? `${config.schedule.min}—${config.schedule.max}` : config.schedule.interval;
  ...
  dial.setAttribute('aria-valuemin', minimum);
...
function lockControls() {
  ...
  $('#interval-dial').setAttribute('aria-disabled', lock);
...
function renderState() {
  ...
  $('#posted-count').textContent = state.runs.filter(r => r.status === 'posted').length;
  $('#preview-count').textContent = state.runs.filter(r => r.status === 'preview').length;
  $('#failed-count').textContent = state.runs.filter(r => r.status === 'failed').length;
...
async function refresh(initial = false) {
  try {
    const response = await fetch('/api/state'); if (!response.ok) throw new Error('Disconnected'); const data = await response.json();
    ...
    renderState();
  } catch (error) { connected = false; $('#connection').innerHTML = '<b class="led"></b> ENGINE OFFLINE'; $('#feed-status').textContent = 'FEED OFFLINE'; $('#footer-connection').textContent = 'LOCAL ENGINE · OFFLINE'; $('#monitor-status').textContent = 'DISCONNECTED · COUNTS MAY BE STALE'; if (initial) toast('Cannot connect to the local engine. Start npm start and reload.', true); lockControls(); }
}
function inputField(name, label, value = '', type = 'text') { return `<label>${label}<input name="${name}" type="${type}" value="${escape(value)}" ${type === 'text' ? 'required' : ''}></label>`; }
function textField(name, label, value = '', help = '') { return `<label>${label}<textarea name="${name}" spellcheck="false">${escape(value)}</textarea>${help ? `<span class="tiny">${help}</span>` : ''}</label>`; }
...
$('#test-board').onclick = () => action(async () => { if (dirty) await save(false); $('#board-result').textContent = 'Dialing the mothership…'; try { const data = await api('test'); $('#board-result').textContent = data.message; toast('Uplink established.'); } catch (e) { $('#board-result').textContent = e.message; throw e; } });
...
$('#clock-buttons').onclick = e => { const button = e.target.closest('[data-clock]'); if (!button || locked()) return; config.schedule.mode = button.dataset.clock; markDirty(); renderClock(); };
$('#lanes').onclick = e => {
  const pad = e.target.closest('[data-route]'); if (!pad || pad.disabled || locked()) return;
  config.selection[pad.dataset.route] = pad.dataset.choice; markDirty();
...
function setDial(value) {
  ...
  config.schedule[dialKey()] = next; $(`#${dialKey()}`).value = next;
...
for (const event of ['pointerup', 'pointercancel', 'blur']) window.addEventListener(event, e => { if (event === 'blur' || e.pointerId === dialDrag?.pointerId) dialDrag = null; });
dial.addEventListener('keydown', e => {
  if (locked()) return;
  const value = config.schedule[dialKey()], step = e.shiftKey ? 60 : 1;
  const values = { ArrowUp: value + step, ArrowRight: value + step, ArrowDown: value - step, ArrowLeft: value - step, PageUp: value + 60, PageDown: value - 60, Home: 10, End: 86400 };
...
$$('[data-tab]').forEach(b => b.onclick = () => { tab = b.dataset.tab; renderLibrary(); });
...
$('#library').onclick = e => {
  ...
  const key = { agents: 'agent', spaces: 'space', personalities: 'personality' }[tab];
...
$('#editor-fields').onchange = e => {
  if (e.target.id !== 'agent-preset' || !e.target.value) return;
  const preset = presets[e.target.value], form = $('#entry-form');
  for (const key of ['command', 'delivery', 'output']) form.elements[key].value = preset[key];
  form.elements.args.value = JSON.stringify(preset.args, null, 2); form.elements.env.value = '{}';
};
$('#entry-form').onsubmit = e => {
  e.preventDefault(); const form = e.target, item = { ...editing, name: form.elements.name.value.trim(), enabled: form.elements.enabled.checked };
  try {
    if (tab === 'personalities') { item.prompt = form.elements.prompt.value; item.color = form.elements.color.value; }
    if (tab === 'spaces') { item.path = form.elements.path.value.trim(); if (!/^(\/|~\/|[A-Za-z]:\\)/.test(item.path)) throw new Error('Use an absolute folder path or ~/.'); }
    if (tab === 'agents') { for (const key of ['command', 'delivery', 'output']) item[key] = form.elements[key].value; item.args = JSON.parse(form.elements.args.value); item.env = JSON.parse(form.elements.env.value); if (!Array.isArray(item.args) || item.args.some(a => typeof a !== 'string')) throw new Error('Arguments must be a JSON array of strings.'); if (!item.env || Array.isArray(item.env) || typeof item.env !== 'object') throw new Error('Environment must be a JSON object.'); }
    const index = config[tab].findIndex(x => x.id === item.id); if (index >= 0) config[tab][index] = item; else config[tab].push(item);
    const key = { agents: 'agent', spaces: 'space', personalities: 'personality' }[tab]; if (!item.enabled && config.selection[key] === item.id) config.selection[key] = 'random';
...
$('#session-list').onclick = e => { const row = e.target.closest('[data-run]'); if (row) renderRun(row.dataset.run); };
